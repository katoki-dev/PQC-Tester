/* ═══════════════════════════════════════════
   QuantaVault PQC API Tester — Application Logic
   ═══════════════════════════════════════════ */

// ─── State ───
let history = JSON.parse(localStorage.getItem('pqc_tester_history') || '[]');
renderHistory();
...
const PRESETS = {
    sign: {
        ...
        body: {
            pqcKeyId: '',
            authKeyId: '',
            data: 'Hello from QuantaVault PQC Tester!'
        }
    },
    verify: {
        method: 'POST',
    ...
    encrypt: {
        ...
        body: {
            pqcKeyId: '',
            authKeyId: '',
            data: 'Secret message for AES-256'
        }
    },
    decrypt: {
        method: 'POST',
    ...
    try {
        ...
        responseBody.innerHTML = syntaxHighlight({
            ...
            error: {
                ...
                details: [
                    { reason: err.message },
                    { hint: 'Is the backend running? check the Base URL and ensure the server is accessible.' },
                    { troubleshooting: 'If the server is running, check for CORS blockers in the browser console.' }
                ]
            }
        });
...
function addToHistory(method, path, status, time) {
    ...
    localStorage.setItem('pqc_tester_history', JSON.stringify(history));
    renderHistory();
}

function clearHistory() {
    history = [];
    localStorage.setItem('pqc_tester_history', '[]');
    renderHistory();
}
...
async function fetchPoliciesForSelector() {
    const baseUrl = document.getElementById('baseUrl').value.replace(/\/$/, '');
    const token = document.getElementById('jwtToken').value.trim();
    if (!token) {
    ...
    try {
        ...
        selector.innerHTML = '<option value="">Select a policy...</option>' + 
            policies.map(p => `<option value="${p.id}">${p.name} (${p.status})</option>`).join('');
        
        console.log('Policies loaded for selector');
    } catch (err) {
        console.error('Failed to fetch policies:', err);
        alert('Failed to fetch policies. Check console.');
    }
}

function applyPolicy() {
    const policyId = document.getElementById('policySelector').value;
    const policy = availablePolicies.find(p => p.id === policyId);
    if (!policy) return;

    const bodyArea = document.getElementById('requestBody');
    try {
        const body = JSON.parse(bodyArea.value);
        body.pqcKeyId = policy.pqcKeyId;
        body.authKeyId = policy.authKeyId;
        bodyArea.value = JSON.stringify(body, null, 2);
    } catch (e) {
        // If invalid JSON, just overwrite with a template
        bodyArea.value = JSON.stringify({
            pqcKeyId: policy.pqcKeyId,
            authKeyId: policy.authKeyId,
            data: "Test Data"
        }, null, 2);
    }
}
